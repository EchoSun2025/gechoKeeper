...
export default function App() {
  ...
  return <main className="device-shell"><div className="ratio-note">GEKO KEEPER · WEB FIRST · LOCAL DATA</div><section className={`app ${active ? 'focus-active' : ''}`}>
    ...
    {page === 'home' && <section className="screen home active"><video className="home-video" autoPlay muted loop playsInline src="/media/gekoSunshine.mp4" /><div className="sun-overlay" /><header className="home-header"><div><span className="eyebrow">GOOD AFTERNOON</span><h1>Wellington {date()} ☁</h1></div></header><div className="home-message"><span>Take it gently today. One good thing is enough.</span><b>— Geko</b></div><div className="home-actions"><div className="sound-wrap"><button className="round-button bird" onClick={() => setSound(!sound)} aria-label="Bird sounds">♪</button><audio ref={audio} loop src="/media/bird.mp3" /></div>{!active && <button className="focus-cta" onClick={() => setCreateOpen(true)}><span>F.</span><div><small>READY WHEN YOU ARE</small>Focus</div><i>→</i></button>}</div><div className="swipe-tip">Swipe left to plan your day</div>
      {createOpen && <section className="create-overlay overlay open"><button className="overlay-close" onClick={() => setCreateOpen(false)}>×</button><div className="task-art"><div className="art-ring" /><img src="/media/gekoFighting.png" alt="Geko cheering" /></div><form className="task-form" onSubmit={beginFocus}><span className="eyebrow">FOCUS SESSION</span><label>What would you like to focus on?</label><input value={title} onChange={event => setTitle(event.target.value)} autoFocus /><div className="tags">{tags.map(tag => <button type="button" key={tag.id} onClick={() => setTagId(tag.id)} className={`tag ${tag.id} ${tag.id === tagId ? 'selected' : ''}`} style={{ background: tag.color }}>{tag.name}</button>)}<button type="button" id="newTag" onClick={addTag}>+ New tag</button></div><button className="start-focus" type="submit">Focus <span>→</span></button></form></section>}
      {active && <section className="focus-overlay show"><div className="focus-time"><span>FOCUSING</span><strong>{formatTime(seconds)}</strong></div><div className="focus-task"><span className="tiny-tag" style={{ background: activeTag?.color }}>{activeTag?.name}</span><b>{active.title}</b></div><div className="focus-tools"><button className="focus-tool water" onClick={() => window.alert('Water logged — keep going!')}><span>💧</span><small>Water</small></button><button className="focus-tool move" onClick={() => window.alert('Move break logged — nice work!')}><span>🏃</span><small>Move</small></button><button className="finish" onClick={finishFocus} aria-label="Finish focus"><span className="stop-mark" /></button></div></section>}
    </section>}
